...
const IOSRow: React.FC<{
  ...
  <motion.div
    ...
    className={`w-full flex items-center gap-3 px-4 py-3.5 ${!isLast ? 'border-b border-white/5' : ''} ${
      onClick ? 'active:bg-white/5 cursor-pointer' : ''
    } transition-colors`}
  >
    {icon}
...
const SectionHeader: React.FC<{ title: string }> = ({ title }) => (
  <h2 className="text-xs font-medium text-zinc-500 uppercase tracking-widest mb-2.5 px-1">{title}</h2>
);

/* ── Card wrapper ───────────────────────────────────────────────── */
const Card: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => (
  <div className={`glass card-shadow rounded-3xl overflow-hidden ${className}`}>
    {children}
  </div>
);
...
export const SettingsStats: React.FC<SettingsStatsProps> = ({
  ...
  const minutesPercent = Math.min(100, (todayStats.focusMinutes / dailyTarget.minutes) * 100);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 pt-10 pb-10">
      {/* ── Header ──────────────────────────────────────────── */}
      <motion.div
        ...
        className="mb-8"
      >
        <h1 className="text-3xl font-semibold text-white tracking-tight leading-tight">Settings</h1>
      </motion.div>
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/5">
              ...
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  onChange={(e) => onUpdateSettings({ ...settings, focusMinutes: Number(e.target.value) || 25 })}
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/5">
              ...
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  onChange={(e) => onUpdateSettings({ ...settings, breakMinutes: Number(e.target.value) || 5 })}
                  className="w-14 text-right bg-transparent text-[15px] text-white focus:outline-none tabular-nums"
                />
                <span className="text-[13px] text-zinc-500 w-6">min</span>
              </div>
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/5">
              ...
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  onChange={(e) => onUpdateSettings({ ...settings, longBreakMinutes: Number(e.target.value) || 15 })}
                  className="w-14 text-right bg-transparent text-[15px] text-white focus:outline-none tabular-nums"
                />
                <span className="text-[13px] text-zinc-500 w-6">min</span>
              </div>
            ...
            <div className="px-4 py-3.5 border-b border-white/5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <select
                  ...
                  className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[13px] text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 appearance-none pr-7"
                >
                  {SOUND_NAMES.map((s) => (
              ...
              <div className="flex items-center gap-3 pl-12">
                ...
                <motion.button
                  ...
                  className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-zinc-300 active:bg-white/10 transition-colors"
                >
            ...
            <div className="flex items-center gap-3 px-4 py-3.5">
              ...
              {notifStatus !== 'granted' ? (
                <motion.button
                  ...
                  className="px-3.5 py-1.5 rounded-lg bg-indigo-500/20 text-indigo-300 text-[13px] font-medium active:bg-indigo-500/30 transition-colors"
                >
                  Enable
                </motion.button>
              ) : (
        ...
        <motion.div variants={sectionVariants} className="mb-7">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <motion.div
              ...
              className="glass card-shadow rounded-2xl p-4 flex flex-col items-center"
            >
              <div className="w-9 h-9 rounded-full bg-blue-500/15 border border-blue-500/20 flex items-center justify-center mb-2.5">
                <Target className="w-4 h-4 text-blue-400" />
              </div>
              <span className="text-2xl font-semibold text-white tabular-nums">{todayStats.sessions}</span>
              <span className="text-[11px] text-zinc-500 mt-0.5">Sessions</span>
              <div className="w-full h-1 rounded-full bg-white/5 mt-2.5 overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
            ...
            <motion.div
              ...
              className="glass card-shadow rounded-2xl p-4 flex flex-col items-center"
            >
              <div className="w-9 h-9 rounded-full bg-emerald-500/15 border border-emerald-500/20 flex items-center justify-center mb-2.5">
                <Clock className="w-4 h-4 text-emerald-400" />
              </div>
              <span className="text-2xl font-semibold text-white tabular-nums">{todayStats.focusMinutes}</span>
            ...
            <motion.div
              ...
              className="glass card-shadow rounded-2xl p-4 flex flex-col items-center"
            >
              <div className="w-9 h-9 rounded-full bg-amber-500/15 border border-amber-500/20 flex items-center justify-center mb-2.5">
                <AlertCircle className="w-4 h-4 text-amber-400" />
              </div>
              <span className="text-2xl font-semibold text-white tabular-nums">{todayStats.distractions}</span>
          ...
          <Card>
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/5">
              ...
              <input
                ...
                onChange={(e) => onUpdateDailyTarget({ ...dailyTarget, minutes: Number(e.target.value) || 120 })}
                className="w-14 text-right bg-transparent text-[15px] text-white focus:outline-none tabular-nums"
              />
            </div>
            ...
            {syncCode && (
              ...
              >
                <div className="glass rounded-2xl p-4 flex items-center justify-between">
                  <div>
                    <div className="text-[11px] text-zinc-500 uppercase tracking-widest mb-1">Your Sync Code</div>
                    <div className="text-lg font-mono font-semibold text-white tracking-widest">{syncCode}</div>
                  </div>
                  <motion.button
                    whileTap={{ scale: 0.9 }}
                    onClick={handleCopyCode}
                    className="w-9 h-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-zinc-300 active:bg-white/10 transition-colors"
                  >
                ...
                <motion.button
                  ...
                  className="w-full mt-3 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[13px] font-medium active:bg-rose-500/20 transition-colors"
                >
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-[15px] text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 font-mono tracking-wider uppercase"
                  />
                  <motion.button
                    whileTap={{ scale: 0.95 }}
                    onClick={async () => {
                    ...
                    className="px-4 py-2.5 rounded-xl bg-indigo-500 text-white text-[13px] font-semibold active:bg-indigo-600 transition-colors flex items-center gap-1.5"
                  >
                ...
                <motion.button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/15 border border-emerald-500/20 text-emerald-400 text-[13px] font-medium active:bg-emerald-500/25 transition-colors"
                >
